...
function renderBlock(term, items, tokens) {
  ...
  if (!items.length) {
    ...
    // Atualiza índice por arquivo
    updateResultsIndexFromDOM();
  }
...
/* ---------- Índice por arquivo (Civil, Penal, etc.) ---------- */
function normalizeSourceKey(label) {
  return (label || "")
    .toLowerCase()
...
function updateResultsIndexFromDOM() {
  // Coletar todos os cards renderizados com data-source
  const cards = els.stack ? Array.from(els.stack.querySelectorAll(".card")) : [];
  const groups = new Map(); // key -> { label, count, firstCard }
  for (const c of cards) {
    const label = c.getAttribute("data-source");
    if (!label) continue;
    const key = normalizeSourceKey(label);
    if (!groups.has(key)) groups.set(key, { label, count: 0, firstCard: c });
    groups.get(key).count += 1;
  }
  // esconder se < 2 fontes
  if (groups.size < 2) {
    els.idxWrap?.setAttribute("hidden","true");
    els.idxPanel?.setAttribute("hidden","true");
    els.idxSheet?.setAttribute("hidden","true");
    els.idxToggle?.setAttribute("aria-expanded","false");
    return;
  }
  // Marcar âncoras no primeiro card de cada grupo
  for (const [key, g] of groups) {
    if (!g.firstCard.id) g.firstCard.id = "idx-" + key;
  }
  // Ordenar por count desc
  const arr = Array.from(groups.entries()).sort((a,b)=>b[1].count - a[1].count);
  // Preencher painel (desktop)
  if (els.idxPanel) {
    els.idxPanel.innerHTML = "";
    for (const [key, g] of arr) {
      const a = document.createElement("button");
      a.className = "index-item";
      a.setAttribute("role","listitem");
      a.dataset.sourceKey = key;
      a.textContent = `${g.label} (${g.count})`;
      a.addEventListener("click", () => activateIndexItem(key));
      els.idxPanel.appendChild(a);
    }
  }
  // Preencher lista da sheet (mobile)
  if (els.idxList) {
    ...
    for (const [key, g] of arr) {
      ...
      li.dataset.sourceKey = key;
      li.textContent = `${g.label} (${g.count})`;
      li.addEventListener("click", () => {
        closeIndexSheet();
        activateIndexItem(key);
      });
      els.idxList.appendChild(li);
    }
  }
  // Mostrar wrapper
  els.idxWrap?.removeAttribute("hidden");
}

function activateIndexItem(sourceKey) {
  const anchor = document.getElementById("idx-" + sourceKey);
  if (!anchor) return;
  closeIndexPanel();
  // Ajuste de offset da topbar
...
function openIndexPanel() {
  els.idxPanel?.removeAttribute("hidden");
  els.idxToggle?.setAttribute("aria-expanded","true");
}
...
function openIndexSheet() {
  if (!els.idxSheet) return;
  els.idxSheet.removeAttribute("hidden");
  els.idxSheet.setAttribute("aria-hidden","false");
  document.documentElement.style.overflow = "hidden";
}
function closeIndexSheet() {
  if (!els.idxSheet) return;
  els.idxSheet.setAttribute("hidden","true");
  els.idxSheet.setAttribute("aria-hidden","true");
  document.documentElement.style.overflow = "";
}

/* inicializar listeners do índice */
function initIndexUI() {
  els.idxToggle?.addEventListener("click", () => {
    // desktop → dropdown; mobile → sheet
    const isMobile = window.matchMedia("(max-width: 768px)").matches;
    if (isMobile) openIndexSheet();
    else {
      if (els.idxPanel?.hasAttribute("hidden")) openIndexPanel();
      else closeIndexPanel();
    }
  });
  els.idxSheetClose?.addEventListener("click", () => closeIndexSheet());
  els.idxSheet?.querySelector(".sheet-backdrop")?.addEventListener("click", () => closeIndexSheet());

  // Observador para destacar item ativo (opcional simples: nada por enquanto)
}
