...
function getCurrentQuery(){
  ...
  const fromDock = (dockInput && dockInput.value || '').trim();
  if (fromDock) return fromDock;
  const fromTop = (els.q && els.q.value || '').trim();
  return fromTop;
}
...
async function doSearchFromTerm(termRaw){ const prevQ=els.q; if(prevQ){ els.q.value = termRaw; } return doSearch(); }

async function doSearch(){
  const termRaw = getCurrentQuery();
  if (!termRaw) return;
...
`;
  ...
  btnVerTexto.className = "mini";
  btnVerTexto.textContent = "Ver Texto";
  btnVerTexto.addEventListener("click", ()=>{
    card.querySelector(".card-body")?.scrollIntoView({behavior:'smooth', block:'center'});
  });

  const btnVerCodigo = document.createElement("button");
  btnVerCodigo.className = "mini";
  btnVerCodigo.textContent = "Ver Código";
  btnVerCodigo.addEventListener("click", ()=> openReader(item));

  const btnGoogle = document.createElement("button");
  btnGoogle.className = "mini";
  ...
  mini.append(btnVerTexto, btnVerCodigo, btnGoogle);

  dot.addEventListener("click", (e)=>{
    e.stopPropagation();
    mini.hidden = !mini.hidden;
    dot.setAttribute("aria-expanded", String(!mini.hidden));
    if (!mini.hidden) {
      const rect = dot.getBoundingClientRect();
      mini.style.position = "fixed";
      mini.style.left = (rect.left - 6) + "px";
      mini.style.top = (rect.top - 44) + "px";
      document.body.appendChild(mini);
    }
  });
  document.addEventListener("click", ()=>{ if(!mini.hidden){ mini.hidden = true; dot.setAttribute("aria-expanded","false"); }});

  actionsWrap.append(chk, dot);
...
const dockInput = document.getElementById("dockInput");
if (dockForm && dockInput) {
  dockForm.addEventListener("submit", (e)=>{ e.preventDefault(); const q=(dockInput.value||'').trim(); if(q){ doSearchFromTerm(q); }});
  dockInput.addEventListener("keydown",(e)=>{ if(e.key==='Enter'){ e.preventDefault(); const q=(dockInput.value||'').trim(); if(q){ doSearchFromTerm(q); }}});
}

/* Build found-filter after results */
function buildFoundFilter() {
  const wrap = document.getElementById("foundFilter");
  const btn = document.getElementById("foundFilterBtn");
  const list = document.getElementById("foundFilterList");
  if (!wrap || !btn || !list) return;

  const cards = Array.from(document.querySelectorAll(".card"));
  const sources = new Map();
  cards.forEach((c)=>{
    const src = c.getAttribute("data-source") || c.querySelector(".pill")?.textContent?.trim() || "Arquivo";
    if (!sources.has(src)) sources.set(src, c);
  });

  if (!sources.size) { wrap.hidden = true; return; }

  list.innerHTML = "";
  sources.forEach((firstCard, label)=>{
    const li = document.createElement("li");
  ...
  btn.textContent = `Arquivos encontrados (${sources.size})`;
  wrap.hidden = false;
