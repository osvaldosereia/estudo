...
els.studyBtn?.addEventListener("click", async () => {
  ...
  buildMiniList(els.questionsList, state.studyIncluded);
  showModal(els.studyModal);

  const activeTab = document.querySelector(".tab-btn.active")?.dataset.tab || "prova";

  if (activeTab === "questoes") {
    const prompt = await buildQuestionsPrompt(state.studyIncluded);
    copyToClipboard(prompt);
