...
if (!document.getElementById("clearSelectedBtn")) {
  ...
  // criar nova base de botões alinhados à direita
  ...
  // botão 'ver código' (abre o modal igual ao título)
  const openBtn = document.createElement("button");
  openBtn.className = "btn soft";
  openBtn.textContent = "Ver Código";
  openBtn.addEventListener("click", () => openModal(id));

  // botão 'ver texto' (já existente)
  const btnVer = left.querySelector("button");
  const btnPlanalto = left.querySelector("a");

  // montar rodapé com: ver texto / ver código / planalto / checkbox
  footer.append(btnVer, openBtn, btnPlanalto, chk);
