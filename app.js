/* ==========================
   direito.love — app.js (SAFE PATCH)
   ========================== */
...
function planaltoUrlFor(item){
  ...
  if (base && art){ return base; } // âncoras variam; abre a lei base
...
function bagHasTokenWord(bag, token){ const t = token.normalize("NFD").replace(/\p{M}/gu,""); return new RegExp(`\\b${t}\\b`, "i").test(bag); }
...
function renderCard(item, tokens=[], ctx={context:"results"}){
  ...
  body.className="body is-collapsed"; body.innerHTML=truncatedHTML(item.text, tokens);
  body.style.cursor="pointer"; body.addEventListener("click", ()=> openReader(item));
  ...
  consult.className="toggle alt"; consult.textContent="consultar"; consult.href="#";
  ...
  chk.className="chk"; chk.setAttribute("aria-label","Selecionar bloco");
...
function renderSelected(){
  ...
  for (const [_, a] of state.selected){
    const row = document.createElement("article"); row.className="article"; row.id=a.htmlId;
    const chk = document.createElement("button"); chk.className="chk a-chk"; chk.setAttribute("aria-label","Selecionar bloco");
    chk.innerHTML=`<svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true"><path d="M5 13l4 4L19 7" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
    const sync=()=>{ chk.dataset.checked = state.selected.has(a.id) ? "true":"false"; }; sync();
    chk.addEventListener("click", ()=>{
      if (state.selected.has(a.id)){ state.selected.delete(a.id); toast(`Removido (${state.selected.size}/${MAX_SEL}).`); row.remove(); updateBottom(); }
      else { if (state.selected.size>=MAX_SEL){ toast("⚠️ Limite de 6 blocos."); return; } state.selected.set(a.id,{...a}); toast(`Adicionado (${state.selected.size}/${MAX_SEL}).`); }
      sync();
    });
    const h = document.createElement("div"); h.innerHTML = `<div class="pill">${a.source}</div><div class="body">${escHTML(a.title)}</div>`;
    row.append(h, chk); els.selectedStack.appendChild(row);
  }
}
...
els.studyBtn?.addEventListener("click", ()=>{ renderSelected(); if (els.studyModal) els.studyModal.hidden=false; });
...
els.includeObsBtn?.addEventListener("click", ()=>{
  ...
  const li = document.createElement("li"); li.textContent = v; els.questionsList.appendChild(li);
