...
/* ---------- Estudar (menu) ---------- */
// Abre/fecha menu suspenso ao clicar no botão Estudar
els.studyBtn?.addEventListener("click", (e) => {
  // Se não há selecionados, mantém desabilitado (só feedback visual)
  if (!state.selected.size) return;
  if (!els.studyMenu) return;
  els.studyMenu.hidden = !els.studyMenu.hidden;
});

// Fechar o menu se clicar fora
document.addEventListener("click", (e) => {
  const withinDropdown = e.target.closest?.(".dropdown");
  if (!withinDropdown && els.studyMenu && !els.studyMenu.hidden) {
    els.studyMenu.hidden = true;
  }
});

// Atalhos do menu Estudar
document.querySelectorAll(".study-opt").forEach((btn) => {
  btn.addEventListener("click", async () => {
    if (!state.selected.size) { toast("Selecione pelo menos 1 bloco."); return; }
    const action = btn.dataset.action;

    if (action === "rapido" || action === "pesquisar") {
      ...
      copyToClipboard(prompt);
    } else if (action === "comparativo") {
      if (state.selected.size < 2) { toast("Selecione pelo menos 2 blocos."); return; }
      state.compareIncluded = new Set([...state.selected.keys()]);
      buildMiniList(els.compareList, state.compareIncluded);
      showModal(els.compareModal);
      const prompt = await buildComparePrompt(state.compareIncluded);
      copyToClipboard(prompt);
    }
...
/* ---------- Comparativo ---------- */
async function loadCompareTemplate() {
  const PATH = "data/prompts/prompt_comparativo.txt";
  try {
    const r = await fetch(PATH, { cache: "no-cache" });
    if (!r.ok) throw new Error();
    return (await r.text()).trim();
  } catch {
    toast("Não encontrei data/prompts/prompt_comparativo.txt");
    return "Você é uma I.A. jurídica. Compare, contraste e relacione criticamente os blocos abaixo, destacando semelhanças, diferenças, conflitos aparentes, aplicação prática e pegadinhas de prova.";
  }
}
async function buildComparePrompt(includedSet) {
  const tpl = await loadCompareTemplate();
...
els.compareUpdate?.addEventListener("click", async () => {
  const prompt = await buildComparePrompt(state.compareIncluded);
  copyToClipboard(prompt);
  toast("Lista atualizada e prompt copiado.");
});
els.copyCompareBtn?.addEventListener("click", async () => {
  const prompt = await buildComparePrompt(state.compareIncluded);
  copyToClipboard(prompt);
});

// Fechar modal Comparar
document.addEventListener("click", (e) => {
  if (e.target.matches("[data-close-compare]")) hideModal(els.compareModal);
});
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape" && els.compareModal && !els.compareModal.hidden) {
    hideModal(els.compareModal);
  }
});
