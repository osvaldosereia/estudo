...
/* Hook: atualiza índice após renderBlock */
(function(){
  const tryHook = () => {
    if (typeof renderBlock === "function") {
      const _orig = renderBlock;
      window.renderBlock = function(...args){
        const r = _orig.apply(this, args);
        try { window.__rebuildSearchIndex && window.__rebuildSearchIndex(); } catch(e){}
        return r;
      };
    }
  };
