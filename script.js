...
const modalIAs = document.querySelector('#modalIAs');
...
function pushUser(text){
  const node = el(`<div class="msg user"><div class="bubble">${text}</div><div class="avatar">👤</div></div>`);
  app.appendChild(node); app.scrollTo({top: app.scrollHeight, behavior:'smooth'});
  return node;
}
...
async function searchByArticle(codeId, articleNum) {
  ...
  const node = data[key];
  if (!node) return [];
  // Retorna apenas as perguntas (sem título), conforme pedido
    ...
    const msg = el(`<div class="msg bot"><div class="avatar">🤖</div>
      <div class="bubble">
        ...
        <div style="margin-top:6px">
          <button class="chip" data-i="${i}" data-role="toggle">➕ Incluir</button>
        </div>
      </div></div>`);
    app.appendChild(msg);
...
function togglePergunta(text, btn, msgNode){
  ...
  if (i>=0){ state.selecionadas.splice(i,1); btn.textContent='➕ Incluir'; msgNode.querySelector('.bubble').style.outline='none'; }
  else { state.selecionadas.push(text); btn.textContent='✔️ Remover'; msgNode.querySelector('.bubble').style.outline='2px solid var(--brand)'; }
...
async function gotoEstrategias(){
  ...
  const map = new Map(); state.estrategias.forEach(es=>map.set(es.id, es));
...
Regras: linguagem simples, sem juridiquês excessivo; quando possível, traga entendimento doutrinário majoritário e prática forense. Ao final, inclua um checklist enxuto de revisão.`;
  ...
  node.querySelector('#btnCopiar').addEventListener('click', async ()=>{
