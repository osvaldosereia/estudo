...
function renderCodeSelect(){
  ...
  appEls.selCodigo.innerHTML =
    `<option value="" selected disabled>Selecione…</option>` + optsGroups;

  // Deixa sem seleção por padrão (força o usuário a escolher)
  state.codigo = null;
}
...
async function onBuscar(){
  ...
  if (!codeId){
    appEls.resultChips.innerHTML = '';
    appEls.resultMsg.textContent = 'Selecione um Código/Lei antes de buscar.';
    return;
  }
...
function start(){ renderCodeSelect(); bind(); }
